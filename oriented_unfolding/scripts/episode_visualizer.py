...
import datetime
import os
os.chdir('/home/new_heruhan/gnq/oriented-unfolding/oriented_unfolding')
import pathlib
...
from oriented_unfolding.pc_vis import *
import pandas as pd

from skimage import draw
from oriented_unfolding.notebooks.visualization_utils import *
    ...
    with FileLock(dataset_path + ".lock"):
        with h5py.File(dataset_path, "r") as dataset:
            ...
            for i, k in enumerate(keys):
                ...
                # preaction_weighted_distance, preaction_cover_reward,preaction_icp_cost, _, _, preaction_clouds = deformable_distance(np.array(init_verts), np.array(preaction_verts), group.attrs['max_coverage'])
                # postaction_weighted_distance, postaction_cover_reward,postaction_icp_cost, _, _, postaction_clouds = deformable_distance(np.array(init_verts), np.array(postaction_verts), group.attrs['max_coverage'])
                _, _, _, _, _, preaction_clouds = deformable_distance(
                    np.array(init_verts), np.array(preaction_verts), group.attrs['max_coverage'])
                _, _, _, _, _, postaction_clouds = deformable_distance(
                    np.array(init_verts), np.array(postaction_verts), group.attrs['max_coverage'])
                ...
                if visualize_metrics:
                    pretransform_obs = draw_text(pretransform_obs, text=f'Rigid: {group.attrs["preaction_orient_reward"]:.3f}', org=(20, 45), font_scale=1)
                    pretransform_obs = draw_text(pretransform_obs, text=f'Deformable: {group.attrs["preaction_cover_reward"]:.3f}', org=(20, 70), font_scale=1)

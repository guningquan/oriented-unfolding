...
import numpy as np
import pathlib
from oriented_unfolding.utils.env_utils import (
    ...
    find_max_indices,
    shirt_keypoints)
from oriented_unfolding.learning.utils import deformable_distance
import torch
from .exceptions import MoveJointsException
from oriented_unfolding.learning.utils import prepare_image
from typing import List, Callable
from itertools import product
import math
from oriented_unfolding.environment.flex_utils import (
    set_scene,
    get_image,
...
from oriented_unfolding.tasks.generate_tasks import Task
...
import wandb
# from oriented_unfolding.keypoint_detector.keypoint_inference import KeypointDetector

def convert_float32_to_float(data):
    if isinstance(data, np.float32):
        return float(data)
    elif isinstance(data, tuple):
        return tuple(convert_float32_to_float(x) for x in data)
...
class SimEnv:
    def __init__(self,
        ...
        seed_all(seed)
        # print("kwargs['eval'] is",kwargs['eval'])
        self.is_eval=kwargs['eval']
        ...
        self.fling_speed_min = fling_speed
        self.fling_speed_max = fling_speed * 2
    ...
    def lift_cloth(self,
        ...
        while True:
            # print("the heights.min() is", heights.min())
            if heights.min() < 0.03:
                fling_height += 0.05
    ...
    def pick_and_fling_primitive(
        ...
        # # # # # # # # # # # # # # # # ## # # # # # # # # # #
        fling_speed_modify = ((min(1.2, fling_height) - PRE_FLING_HEIGHT) / PRE_FLING_HEIGHT) * (
                self.fling_speed_max - self.fling_speed_min) + self.fling_speed_min

        self.fling_speed = fling_speed_modify
        # # # # # # # # # # # # # ## # # # # # # # # # # # # # # #
    ...
    def deformable_reward(self, log_prefix=None):
        ...
        _, _, _, real_cover_reward, rotate_cloth, _ = \
        ...
        cover_reward = coverage
        orient_reward = 1 - np.square(np.tanh( abs(rotate_cloth[1])*0.35 + abs(rotate_cloth[0]) * 0.65))   # reward
        # orient_reward = 1 - np.square(np.tanh(max(abs(rotate_cloth[1]) * 0.5, abs(rotate_cloth[0]))))
        weighted_distance = self.deformable_weight * cover_reward + (1 - self.deformable_weight) * orient_reward

        deformable_dict = {
            "weighted_distance": weighted_distance,
            "cover_reward": cover_reward,
            "orient_reward": orient_reward,
            "pointwise_distance": real_cover_reward,
            "iou": iou,
            "coverage": coverage ,
            "rotate_cloth": rotate_cloth
        }
        ...
        return cover_reward, orient_reward, weighted_distance
    ...
    def step(self, value_maps):
        ...
        current_verts = (pyflex.get_positions().reshape((-1, 4))[:self.num_particles, :3]).copy()
        current_verts[:, 1] = 0
        reverse_goal_verts = (self.init_pos).copy()
        reverse_goal_verts[:, 1] = 0
        R, t = rigid_transform_3D(reverse_goal_verts.T, current_verts.T)
        rotate_cloth = rotateMatrixToEulerAngles(np.array(R))
        ...
        self.terminate = self.terminate or \
                         ...
                         self.is_frame_empty or \
           (self.is_eval and coverage>=0.94 and abs(rotate_cloth[1])<=0.17453) # modiy

        unfolded_result = {"step": self.current_timestep,
                           "coverage": coverage,
                           "rotate_cloth": rotate_cloth}
        unfolded_result = convert_float32_to_float(unfolded_result)
        self.result.append(unfolded_result)
        if self.terminate and self.is_eval:
            pass

        # print(self.is_eval)
        # print("the (coverage>0.9 and rotate_cloth[0]<0.1 and abs(rotate_cloth[1])<=0.1111)",coverage>0.9 and rotate_cloth[0]<0.1 and abs(rotate_cloth[1])<=0.1111)
        # input("check and input enter")
    ...
    def generate_transformed_obs(self, obs):
        ...
        def world_to_pixel(target_position, depth_im, pose_matrix, fov=39.5978, depth_scale=1):
            intrinsics_matrix = compute_intrinsics(fov, depth_im.shape[0])
            point_3d = np.dot(np.linalg.inv(pose_matrix), np.append(target_position, 1.0).reshape(4, 1))
            point_3d = point_3d[:3, 0]
            point_3d[0] = -point_3d[0]
            click_x = (point_3d[0] * intrinsics_matrix[0, 0] / point_3d[2]) + intrinsics_matrix[0, 2]
            click_y = (point_3d[1] * intrinsics_matrix[1, 1] / point_3d[2]) + intrinsics_matrix[1, 2]
            pixel_x = int(round(click_x))
            pixel_y = int(round(click_y))
        ...
        # pp
        all_keypoints_group = self.all_keypoints
        all_keypoints_pos = pyflex.get_positions().reshape((-1, 4))[:self.num_particles, :3]
        pretransform_depth = self.pretransform_depth
        pose_matrix = compute_pose(pos=[0, 2, 0], lookat=[0, 0, 0], up=[0, 0, 1])

        # fling
        # all_keypoints_group_pos = np.zeros((3, self.all_keypoints.shape[1], 3))
        # for m in range(self.all_keypoints.shape[1]):
        #     all_keypoints_group_pos[0][m]  = (all_keypoints_pos[self.all_keypoints[0][m]] + all_keypoints_pos[self.all_keypoints[1][m]])/2
        #     all_keypoints_group_pos[1][m]  = (all_keypoints_pos[self.all_keypoints[2][m]] + all_keypoints_pos[self.all_keypoints[3][m]])/2
        #     all_keypoints_group_pos[2][m]  = (all_keypoints_pos[self.all_keypoints[4][m]] + all_keypoints_pos[self.all_keypoints[5][m]])/2
        #
        all_keypoints_group_pos = np.zeros((7, self.all_keypoints.shape[1], 3))
        for m in range(self.all_keypoints.shape[1]):
            all_keypoints_group_pos[0][m]  = (all_keypoints_pos[self.all_keypoints[0][m]] + all_keypoints_pos[self.all_keypoints[1][m]])/2
            all_keypoints_group_pos[1][m]  = (all_keypoints_pos[self.all_keypoints[2][m]] + all_keypoints_pos[self.all_keypoints[3][m]])/2
            all_keypoints_group_pos[2][m]  = (all_keypoints_pos[self.all_keypoints[4][m]] + all_keypoints_pos[self.all_keypoints[5][m]])/2
            all_keypoints_group_pos[3][m]  = (all_keypoints_pos[self.all_keypoints[0][m]] + all_keypoints_pos[self.all_keypoints[2][m]])/2
            all_keypoints_group_pos[4][m]  = (all_keypoints_pos[self.all_keypoints[0][m]] + all_keypoints_pos[self.all_keypoints[4][m]])/2
            all_keypoints_group_pos[5][m]  = (all_keypoints_pos[self.all_keypoints[1][m]] + all_keypoints_pos[self.all_keypoints[3][m]])/2
            all_keypoints_group_pos[6][m]  = (all_keypoints_pos[self.all_keypoints[1][m]] + all_keypoints_pos[self.all_keypoints[5][m]])/2
        all_keypoints_group_pos =  all_keypoints_group_pos.reshape(-1,3)

        all_keypoints_list = all_keypoints_group.flatten()
        matrix=torch.zeros((2,len(self.rotations)*len(self.adaptive_scale_factors), self.obs_dim, self.obs_dim))
        # matrix = matrix.cpu()
        # matrix=np.zeros((2,len(self.rotations)*len(self.adaptive_scale_factors), self.obs_dim, self.obs_dim))
        # print("matrix device", matrix.device)
        # print("vmap device", vmap.device)
        rotate_scales = 0
        for i in range(len(self.rotations)):
            for j in range(len(self.adaptive_scale_factors)):
                mat = get_transform_matrix(
                    original_dim=pretransform_depth.shape[0],
                    resized_dim=self.obs_dim,  # transformed_depth.shape[0]
                    rotation= -self.rotations[i],  # TODO bug
                    scale=self.adaptive_scale_factors[j])
                inv_mat = np.linalg.inv(mat)
                # pp
                for k in range(len(all_keypoints_list)):
                    transfer_before = all_keypoints_pos[all_keypoints_list[k]]
                    transfer_after = world_to_pixel(target_position=transfer_before, depth_im=pretransform_depth,
                                                    pose_matrix=pose_matrix)
                    transfer_after = np.array([transfer_after, transfer_after])
                    transfer_after_resize = np.matmul(
                        np.concatenate((transfer_after, np.ones((transfer_after.shape[0], 1))), axis=1),
                        inv_mat)[:, :2].astype(int) + 1
                    transfer_after_resize = transfer_after_resize[0]
                    if all(0 <= x < self.obs_dim for x in transfer_after_resize):
                        matrix[1,rotate_scales, transfer_after_resize[0], transfer_after_resize[1]] = 1
                # fling
                for m in range(len(all_keypoints_group_pos)):
                    transfer_before = all_keypoints_group_pos[m]
                    transfer_after = world_to_pixel(target_position=transfer_before, depth_im=pretransform_depth,
                                                    pose_matrix=pose_matrix)
                    transfer_after = np.array([transfer_after, transfer_after])
                    transfer_after_resize = np.matmul(
                    ...
                    transfer_after_resize = transfer_after_resize[0]
                    if all(0 <= x < self.obs_dim for x in transfer_after_resize):
                        matrix[0,rotate_scales, transfer_after_resize[0], transfer_after_resize[1]] = 1
                rotate_scales += 1
        retval['keypoint_mask'] = matrix
    ...
    def reset(self):
        ...
        self.all_keypoints = self.current_task.all_keypoint_groups

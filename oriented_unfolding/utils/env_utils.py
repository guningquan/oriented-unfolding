...
from oriented_unfolding.learning.utils import rewards_from_group
import trimesh
import pyflex
...
from oriented_unfolding.learning.nocs_unet_inference import nocs_pred, get_flow_correspondence
import torch
import cv2
from oriented_unfolding.learning.utils import rewards_from_group
...
def plot_before_after(group, fontsize=16, output_path=None, step=None):
    ...
    def log_data(image, prefix, colors=None):
        ...
        cv2.putText(image, f"R: {group.attrs[f'{prefix}_orient_reward']:.3f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.75, colors[0], 2)
        cv2.putText(image, f"D: {group.attrs[f'{prefix}_cover_reward']:.3f}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.75, colors[1], 2) 
    ...
    comparisons = [(group.attrs[f'postaction_{d}'] < group.attrs[f'preaction_{d}']) for d in ['orient_reward', 'cover_reward', 'weighted_distance', 'pointwise_distance']]
    ...
    # axs[0].set_title(f"[{step}] W: {group.attrs['preaction_weighted_distance']:.3f}, D: {group.attrs['preaction_cover_reward']:.3f} ", fontsize=fontsize)

    axs[1].imshow(postaction_img)
    # axs[1].set_title(f"[{step}] W: {(group.attrs['postaction_weighted_distance']):.3f}, D: {group.attrs['postaction_cover_reward']:.3f}", fontsize=fontsize)

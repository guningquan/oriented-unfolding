...
from oriented_unfolding.environment import SimEnv
from oriented_unfolding.tasks.generate_tasks import TaskLoader
from oriented_unfolding.learning.nets import MaximumValuePolicy, ExpertGraspPolicy
from oriented_unfolding.learning.utils import GraspDataset, rewards_from_group
from oriented_unfolding.utils.env_utils import plot_before_after
    ...
    with FileLock(dataset_path + ".lock"):
        with h5py.File(dataset_path, "r") as dataset:
            ...
            for primitive in action_primitives:
                ...
                gt_deformable_value = step_df[step_df.action_primitive == primitive]['preaction_cover_reward'] - step_df[step_df.action_primitive == primitive]['postaction_cover_reward']
                gt_deformable_value /= DELTA_WEIGHTED_REWARDS_STD

                gt_rigid_value = step_df[step_df.action_primitive == primitive]['preaction_orient_reward'] - step_df[step_df.action_primitive == primitive]['postaction_orient_reward']
            ...
            best_step_df = step_df.groupby('episode').agg({ 'task_difficulty': 'first',
                                                            ...
                                                            'episode':'last', \
                                                            'preaction_iou':'max',
                                                            'preaction_cover_reward':'min',
                                                            'preaction_orient_reward':'min',
                                                            'preaction_pointwise_distance':'min',
                                                            'postaction_iou':'max',
                                                            'postaction_cover_reward':'min',
                                                            'postaction_orient_reward':'min',
            ...
            LEGACY_METRICS_MAP = {
                'deformable_distance': 'cover_reward',
                'rigid_distance': 'orient_reward',
                'weighted_distance': 'weighted_distance',
                'cover_reward': 'pointwise_distance',
                'coverage': 'coverage',
                'iou': 'iou',
            }
...
if __name__ == '__main__':
    ...
    collect_stats('/home/robots18/gnq/oriented-unfolding/experiments/06-10-1546-longsleeve-eval/replay_buffer.hdf5')
...
def rewards_from_group(group):
    ...
    delta_cover_reward = -(group.attrs['preaction_cover_reward'] - group.attrs['postaction_cover_reward']) # modify
    delta_cover_reward /= DELTA_WEIGHTED_REWARDS_STD
    deformable_reward = delta_cover_reward

    delta_orient_reward = -(group.attrs['preaction_orient_reward'] - group.attrs['postaction_orient_reward']) # modify
    delta_orient_reward /= DELTA_WEIGHTED_REWARDS_STD
    rigid_reward = delta_orient_reward
    ...
    weighted_cov_orient = deformable_weight * deformable_reward + (1-deformable_weight) * rigid_reward

    preaction_coverage = group.attrs['postaction_coverage'] - group.attrs['preaction_coverage']

    return {'weighted':torch.tensor(weighted_cov_orient).float(), \
...
def deformable_distance(goal_verts, current_verts, max_coverage, deformable_weight=0.65, flip_x=True, icp_steps=1000, scale=None):
    ...
    real_cover_reward = np.mean(np.linalg.norm(goal_verts - current_verts, axis=1))
    real_cover_reward_flipped = np.mean(np.linalg.norm(flipped_goal_verts - current_verts, axis=1))
    if real_cover_reward_flipped < real_cover_reward:
        real_cover_reward = real_cover_reward_flipped
    ...
    rotate_cloth = rotateMatrixToEulerAngles(np.array(R))  # modify reward
    ...
    cover_reward = min(l2_regular, l2_flipped)

    orient_reward_regular = np.mean(np.linalg.norm(goal_verts - reverse_goal_verts, axis=1))
    orient_reward_flipped = np.mean(np.linalg.norm(flipped_goal_verts - reverse_goal_verts, axis=1))
    orient_reward = min(orient_reward_regular, orient_reward_flipped)

    #make reward scale invariant
    assert(max_coverage != 0 or scale != 0)
    if scale is None:
        cover_reward /= np.sqrt(max_coverage)
        orient_reward /= np.sqrt(max_coverage)
        real_cover_reward /= np.sqrt(max_coverage)
    else:
        cover_reward /= scale
        orient_reward /= scale
        real_cover_reward /= scale

    weighted_distance = deformable_weight * cover_reward + (1 - deformable_weight) * orient_reward

    return weighted_distance, cover_reward, orient_reward, real_cover_reward, rotate_cloth, {"init_vert_cloud": goal_vert_cloud, "normal_init_vert_cloud": normal_init_vert_cloud , "verts_cloud": verts_cloud, 'icp_verts_cloud': icp_verts_cloud, "reverse_init_verts_cloud": reverse_goal_cloud} # modify
...
class GraspDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        with h5py.File(self.hdf5_path, "r") as dataset:
            ...
            weighted_cov_orient, deformable_reward, rigid_reward, l2_reward = \
            ...
            retval = {
                'obs': obs,
                'action': action,
                'weighted_cov_orient': weighted_cov_orient,

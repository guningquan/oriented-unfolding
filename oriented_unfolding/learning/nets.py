...
import ray
import numpy as np
from oriented_unfolding.learning.utils import generate_positional_encoding, shift_tensor
from oriented_unfolding.learning.gui_utils import ExpertDemonstrationWindow
from oriented_unfolding.learning.unet_parts import *
import imutils
...
class MaximumValuePolicy(nn.Module, Policy):
    ...
    def get_action_single(self, state, explore=True):
        ...
        for primitive in self.action_primitives:
            ...
            # # # # # # # # # # modify keypoints # # # # # # # # # #
            matrix = state['keypoint_mask']
            matrix = matrix.to(vmap.device)

            matrix_fling = matrix[0, :, :, :]
            matrix_place = matrix[1, :, :, :]

            if primitive == "fling":
                vmap = torch.where(torch.logical_and(matrix_fling == 1, vmap > 0) == 1, torch.sqrt(vmap), vmap)

            if primitive == "place":
                vmap = torch.where(torch.logical_and(matrix_place == 1, vmap > 0) == 1, torch.sqrt(vmap), vmap)
            # # # # # # # # # # modify keypoints # # # # # # # # # #

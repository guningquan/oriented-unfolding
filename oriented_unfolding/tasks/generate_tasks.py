...
from oriented_unfolding.utils.task_utils import *
from oriented_unfolding.utils.flex_utils import (
    set_scene,
    get_default_config,
    center_object,
    set_to_flatten,
...
def generate_randomization(
    ...
    decision=input("please decide whether to continue: y or n")
    if decision=="y":
        pass
    else:
        print("abort it")
    ...
    # input("Please check center and input enter")
    init_rgb = get_rgb()
    ...
    pyflex.set_positions(curr_verts)
    ...
    # input("Please check the cloth1 and input enter")
    ...
    center_object(num_particle)
    random_translation_vector = np.array([random.uniform(-args.random_translation, args.random_translation), 0,
                                          random.uniform(-args.random_translation, args.random_translation)])
    ...
    # input("Please check the cloth2 and input enter")

    task_rgb = get_rgb()

    heights = pyflex.get_positions().reshape(-1, 4)[:, 1]

    decision_save = input("please decide whether to continue: y or other")
    if decision_save == "y":
        pass
    else:
        print("abort to save it")
        return None
        ...
        if eval:
            ...
            with h5py.File(self.hdf5_path, 'r') as tasks:
                if recreate_task_query is None:
                    ...
                    self.keys = [key for key in tasks if
                                 np.sum(get_init_verts_from_task(tasks[key])) == recreate_task_query]
                    if grid_search:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cloth_mesh_path", type=str,
                        help="path to root dir containing the mesh data",
                        default=f"{os.getcwd()}/oriented_unfolding/cloth_data/cloth3d_pickle")
    ...
    parser.add_argument('--task_difficulty', type=str, default="none", help="(hard, easy, none)",
                        choices=["hard", "easy", "flat", "none", "pick"])

import ray
from oriented_unfolding.utils.utils import (
    setup_envs,
    seed_all,
    setup_network,
    get_loader,
...
from oriented_unfolding.learning.optimization import optimize_fb
...
import h5py
import sys
from oriented_unfolding.scripts.episode_visualizer import visualize_episode
from oriented_unfolding.utils.utils import flatten_dict
...
if __name__ == '__main__':
    ...
    def main(args : DictConfig):
        ...
        # ray.init(local_mode=args.ray_local_mode,num_gpus=4,num_cpus=16)
        seed_all(args.seed)
        ...
        while(True):
            ...
            # print("the datase_size is",dataset_size)
            ...
            if (optimizer is not None) and (dataset_size > args.warmup) and not args.eval:
                ...
                print("the should_update is",should_update)
                if should_update:
                    policy.train()
                        ...
                        loaders = (get_loader(
                                ...
                                filter_fn=lambda attrs:
                                    (attrs['action_primitive'] == action_primitive \
                                        and ('preaction_cover_reward' in attrs)), **args) for action_primitive in args.action_primitives)
                        loader = zip(*loaders)
                ...
                print("the log is",args.log)
                print("the ckpt_paths is",ckpt_paths)
                ...
                print("the policy.train_steps is",policy.train_steps)
                print("the checkpoint_paths is",checkpoint_paths)
